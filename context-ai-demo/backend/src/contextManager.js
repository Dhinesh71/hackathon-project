const { getSession } = require('./memoryStore');
...
const handleMessage = async (sessionId, userMessage) => {
    ...
    // 2. Determine Context for Prompt
    if (isRecallNeeded) {
...
- Be concise, friendly, and direct`;
    ...
    // 5. Update STM with AI response
    session.stm.push({ role: "ai", content: aiResponseText });
    session.messageCount++;

    // 6. Check Summarization Trigger (every 10 messages = 5 user + 5 AI)
